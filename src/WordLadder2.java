...
public class WordLadder2 {
    ...
    public static class Graph {
        ...
        final private List<Set<Integer>> adjOneDirection;
        final private int v;
        Map<String, Integer> wMap = new HashMap<>();
        List<String> wordList;
        public Graph(int vertices, List<String> wordList) {
            this.v = vertices;
            this.wordList = wordList;
            adj = new ArrayList<>(vertices);
            adjOneDirection = new ArrayList<>(vertices);
            for (int i = 0; i < vertices; i++) {
                adj.add(i, new HashSet<>());
                adjOneDirection.add(i, new HashSet<>());
            }
        }
    ...
    public static class ShortestPath {
            ...
            while (!queue.isEmpty()) {
                ...
                for (Integer w : g.adj.get(u)) {
                    if (!visited[w]) {
                        ...
                        dist[w] = dist[u] + 1;
                        pred[w] = u;
                        g.adjOneDirection.get(u).add(w);
                        queue.offer(w);
                    } else if (dist[u] < dist[w]) {
                        g.adjOneDirection.get(u).add(w);
                    }

                    if (w.equals(wMap.get(target))) {
                        g.adjOneDirection.get(u).add(w);
                    }
                }
            }
        ...
        public List<List<String>> paths(String dest) {
            int depth = countPathTo(dest);
            if (depth <= 0) return paths;

            List<Integer> path = new ArrayList<>();
            path.add(wMap.get(src));
            dfs(path, dest, depth);
            return paths;
        }
        ...
        private void dfs(List<Integer> path, String dest, int depth) {
            if (path.get(path.size() - 1).equals(wMap.get(dest))) {
                paths.add(path.stream().map(e -> g.wordList.get(e)).collect(Collectors.toList()));
                path.remove(path.size() - 1);
                return;
            }
            if (dist[path.get(path.size() - 1)] + 1 == depth) {
                path.remove(path.size() - 1);
                return;
            }

            Integer currentNode = path.get(path.size() - 1);
            for (Integer w : g.adjOneDirection.get(currentNode)) {
                path.add(w);
                dfs(path, dest, depth);
            }
            path.remove(path.size() - 1);
    ...
    private boolean isOneCharDiff(String first, String second) {
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        if (beginWord == null || beginWord.length() == 0) return result;
        if (endWord == null || endWord.length() == 0) return result;
        ...
        Graph g = new Graph(wordList.size(), wordList);
        ...
        return shortestPath.paths(endWord);
    }
